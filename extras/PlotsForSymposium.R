...
row <- exposureSummary[exposureSummary$tCohortDefinitionName == "duloxetine" & exposureSummary$cCohortDefinitionName ==
  "Sertraline", ]
...
file.copy(from = file.path(workFolder,
                           "figuresAndtables",
                           "ps",
                           paste0("ps_t", treatmentId, "_c", comparatorId, ".png")),
          to = file.path(symposiumFolder, "Ps.png"),
          overwrite = TRUE)
mean(ps$preferenceScore[ps$treatment == 1] > 0.3 & ps$preferenceScore[ps$treatment == 1] < 0.7)
...
file.copy(from = file.path(workFolder,
                           "figuresAndtables",
                           "controls",
                           paste0("negControls_a1_t", treatmentId, "_c", comparatorId, ".png")),
...
estimate <- calibrated[calibrated$analysisId == 3 & calibrated$targetId == treatmentId & calibrated$comparatorId ==
  comparatorId & calibrated$outcomeId == 2559, ]
print(estimate)
...
ggplot2::ggsave(filename = file.path(symposiumFolder,
                                     "stroke4Db.png"), width = 5, height = 1.5, dpi = 300)
...
estimate <- calibrated[calibrated$analysisId == 3 & calibrated$targetId == treatmentId & calibrated$comparatorId ==
  comparatorId & calibrated$outcomeId %in% outcomes$cohortDefinitionId, ]
estimate <- merge(estimate, outcomes, by.x = "outcomeId", by.y = "cohortDefinitionId")
# d1 <- data.frame(logRr = estimate$logRr, seLogRr = estimate$seLogRr, database = estimate$db,
# outcome = estimate$name, type = 'Uncalibrated')
d2 <- data.frame(logRr = estimate$calLogRr,
                 seLogRr = estimate$calSeLogRr,
                 database = estimate$db,
...
ggplot2::ggsave(filename = file.path(symposiumFolder,
                                     "2nd11Hois4Db.png"), width = 4.5, height = 6, dpi = 300)
...
est <- calibrated[calibrated$analysisId == 3 & calibrated$outcomeId %in% outcomes$cohortDefinitionId, ]
require(ggplot2)
breaks <- c(0.25, 0.5, 1, 2, 4, 6, 8, 10)
...
ggplot(est, aes(x = calLogRr,
                y = calSeLogRr), environment = environment()) + geom_vline(xintercept = log(breaks),
                                                                                         colour = "#AAAAAA",
                                                                                         lty = 1,
                                                                                         size = 0.5) + geom_abline(slope = 1/qnorm(0.025),
                                                                                                                   colour = rgb(0.8, 0, 0),
                                                                                                                   linetype = "dashed",
                                                                                                                   size = 1,
                                                                                                                   alpha = 0.5) + geom_abline(slope = 1/qnorm(0.975), colour = rgb(0.8, 0, 0), linetype = "dashed", size = 1, alpha = 0.5) + geom_point(size = 1, alpha = 0.2, color = rgb(0, 0, 0, alpha = 0.2), shape = 16) + geom_hline(yintercept = 0) + scale_x_continuous("Effect size", limits = log(c(0.25, 10)), breaks = log(breaks), labels = breaks) + scale_y_continuous("Standard Error", limits = c(0, 1)) + theme(panel.grid.minor = element_blank(), panel.background = element_rect(fill = "#FAFAFA", colour = NA), panel.grid.major = element_blank(), axis.ticks = element_blank(), axis.text.y = themeRA, axis.text.x = theme, legend.key = element_blank(), strip.text.x = theme, strip.background = element_blank(), legend.position = "none")
...
for (i in 1:length(rnd)) {
  print(paste("Target: ",
              exposureSummary$tCohortDefinitionName[exposureSummary$tprimeCohortDefinitionId ==
    rnd$tprimeCohortDefinitionId[i]]))
  print(paste("Comparator: ",
              exposureSummary$cCohortDefinitionName[exposureSummary$cprimeCohortDefinitionId ==
    rnd$cprimeCohortDefinitionId[i]]))
  treatmentId <- rnd$tprimeCohortDefinitionId[i]
  comparatorId <- rnd$cprimeCohortDefinitionId[i]
  outcomeId <- rnd$outcomeId[i]
  file.copy(from = file.path(workFolders[i],
                             "figuresAndtables",
                             "ps",
                             paste0("ps_t", treatmentId, "_c", comparatorId, ".png")),
  ...
  cohortMethodDataFolder <- omr$cohortMethodDataFolder[outcomeModelReference$targetId == treatmentId &
    outcomeModelReference$comparatorId == comparatorId & outcomeModelReference$analysisId == 3 &
    outcomeModelReference$outcomeId == outcomeId]
  strataFile <- outcomeModelReference$strataFile[outcomeModelReference$targetId == treatmentId & outcomeModelReference$comparatorId ==
    comparatorId & outcomeModelReference$analysisId == 3 & outcomeModelReference$outcomeId == outcomeId]
  ...
  plotFileName <- file.path(workFolder,
                            "symposium",
                            paste0("balanceScatterPlot_example", i, ".png"))
  CohortMethod::plotCovariateBalanceScatterPlot(balance, fileName = plotFileName)
  ...
  temp <- temp[temp$targetId == treatmentId & temp$comparatorId == comparatorId & temp$analysisId ==
    3 & temp$outcomeId == outcomeId, c("calRr", "calCi95lb", "calCi95ub", "calLogRr", "calSeLogRr")]
  print(temp)
  ...
  themeLA <- element_text(colour = "#000000", size = 12, hjust = 0)
  ggplot(est,
         aes(x = calLogRr, y = calSeLogRr),
         environment = environment()) + geom_vline(xintercept = log(breaks),
                                                                                           colour = "#AAAAAA",
                                                                                           lty = 1,
                                                                                           size = 0.5) + geom_abline(slope = 1/qnorm(0.025),
                                                                                                                     colour = rgb(0.8, 0, 0),
                                                                                                                     linetype = "dashed",
                                                                                                                     size = 1,
                                                                                                                     alpha = 0.5) + geom_abline(slope = 1/qnorm(0.975), colour = rgb(0.8, 0, 0), linetype = "dashed", size = 1, alpha = 0.5) + geom_point(size = 1, alpha = 0.2, color = rgb(0, 0, 0, alpha = 0.2), shape = 16) + geom_point(size = 4, fill = rgb(1, 1, 0), shape = 23, data = temp) + geom_hline(yintercept = 0) + scale_x_continuous("Effect size", limits = log(c(0.25, 10)), breaks = log(breaks), labels = breaks) + scale_y_continuous("Standard Error", limits = c(0, 1)) + theme(panel.grid.minor = element_blank(), panel.background = element_rect(fill = "#FAFAFA", colour = NA), panel.grid.major = element_blank(), axis.ticks = element_blank(), axis.text.y = themeRA, axis.text.x = theme, legend.key = element_blank(), strip.text.x = theme, strip.background = element_blank(), legend.position = "none")
  ggsave(file.path(symposiumFolder,
...
d <- data.frame(x = x, logX = log(x), y = y)

breaks <- c(0.25, 0.5, 1, 2, 4, 6, 8, 10)
theme <- element_text(colour = "#000000", size = 12)
themeRA <- element_text(colour = "#000000", size = 12, hjust = 1)
themeLA <- element_text(colour = "#000000", size = 12, hjust = 0)
ggplot(d, aes(x = logX, y = y), environment = environment()) + geom_vline(xintercept = log(breaks),
                                                                          colour = "#AAAAAA",
                                                                          lty = 1,
                                                                          size = 0.5) + geom_density(stat = "identity",
                                                                                                     color = rgb(0, 0, 0.8),
                                                                                                     fill = rgb(0, 0, 0.8, alpha = 0.5)) + geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + scale_x_continuous("Effect size", limits = log(c(0.25, 10)), breaks = log(breaks), labels = breaks) + theme(panel.grid.minor = element_blank(), panel.background = element_rect(fill = "#FAFAFA", colour = NA), panel.grid.major = element_blank(), axis.ticks = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), axis.text.x = theme, legend.key = element_blank(), strip.text.x = theme, strip.background = element_blank(), legend.position = "none")
...
ggplot(d, aes(x = logX, y = y), environment = environment()) + geom_vline(xintercept = log(breaks),
                                                                          ...
                                                                          size = 0.5) + geom_density(stat = "identity",
                                                                                                     color = rgb(0, 0, 0.8),
                                                                                                     fill = rgb(0, 0, 0.8, alpha = 0.5)) + geom_hline(yintercept = 0) + geom_vline(xintercept = log(2)) + scale_x_continuous("Effect size", limits = log(c(0.25, 10)), breaks = log(breaks), labels = breaks) + theme(panel.grid.minor = element_blank(), panel.background = element_rect(fill = "#FAFAFA", colour = NA), panel.grid.major = element_blank(), axis.ticks = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), axis.text.x = theme, legend.key = element_blank(), strip.text.x = theme, strip.background = element_blank(), legend.position = "none")
